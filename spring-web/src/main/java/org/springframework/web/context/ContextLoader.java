...
public class ContextLoader {
    ...
    public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
        // 1.如果servletContext存在ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE对应的WebApplicationContext，则抛出异常
        ...
        try {
            ...
            // 3.如果WebApplicationContext为空就创建
            if (this.context == null) {
                this.context = createWebApplicationContext(servletContext);
            }
            // 4.如果是ConfigurableWebApplicationContext子类，如果未激活就配置并刷新
            if (this.context instanceof ConfigurableWebApplicationContext) {
                ...
                if (!cwac.isActive()) {
                    ...
                    // 5.配置ConfigurableWebApplicationContext并刷新
                    configureAndRefreshWebApplicationContext(cwac, servletContext);
                }
            }
            // 在servletContext中记录ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE对应的容器
            ...
            // 7.发生异常时，servletContext记录ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE对应的异常，不再初始化
    ...
    protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
        // 1.取得WebApplicationContext类类型
        Class<?> contextClass = determineContextClass(sc);
        // 2.判断WebApplicationContext是否ConfigurableWebApplicationContext
        if (!ConfigurableWebApplicationContext.class.isAssignableFrom(contextClass)) {
            throw new ApplicationContextException("Custom context class [" + contextClass.getName() + "] is not of type [" + ConfigurableWebApplicationContext.class.getName() + "]");
        }
        // 3.创建ConfigurableWebApplicationContext
    ...
    protected Class<?> determineContextClass(ServletContext servletContext) {
        // 1.取得CONTEXT_CLASS_PARAM对应contextClassName
        String contextClassName = servletContext.getInitParameter(CONTEXT_CLASS_PARAM);
        // 2.contextClassName不为空
        ...
        // 3.contextClassName为空
        else {
            // 3.1 从defaultStrategies取得org.springframework.web.context.WebApplicationContext对应配置类路径
    ...
    protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
        // 1.如果ConfigurableWebApplicationContext使用了默认编号，重新设置id
        ...
        // 3.设置ConfigurableWebApplicationContext配置文件路径
        ...
        // 5.自定义globalInitializerClasses,contextInitializerClasses配置初始化
