...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext, DisposableBean {
    ...
    public void refresh() throws BeansException, IllegalStateException {
        // 给容器加锁，避免容器在refresh阶段进行了初始化或者销毁操作
        synchronized (this.startupShutdownMonitor) {
            // Prepare this context for refreshing.
            // 容器刷新前准备，获取容器启动时间，同时设置容器同步标识
            prepareRefresh();

            // Tell the subclass to refresh the internal bean factory.
            // 取得子类刷新后的内部beanFactory并读取配置文件
            ...
            try {
                ...
                // 允许容器的子类注册BeanPostProcessor
                postProcessBeanFactory(beanFactory);

                // Invoke factory processors registered as beans in the context.
                // 调用容器注册的beanFactoryPostProcessor
                invokeBeanFactoryPostProcessors(beanFactory);

                // Register bean processors that intercept bean creation.
                // 注册beanPostProcessor，用于拦截bean创建，在getBean()中调用
                ...
                // 在单例bean初始化之前预留给子类初始化其他特殊bean的入口
                onRefresh();
                ...
                // 设置系统级别的服务，实例化所有非懒加载的单例bean
                finishBeanFactoryInitialization(beanFactory);

                // Last step: publish corresponding event.
                // 初始化完成的回调方法，发布容器刷新完成的事件给监听者
                finishRefresh();
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        // 设置beanFactory的表达式处理器，支持SPEL语法
        beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
        // 为beanFactory添加一个默认的propertyEditor
        beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

        // Configure the bean factory with context callbacks.
        // 当程序定义的bean实现了ApplicationContextAware接口时注入ApplicationContext
        beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
        // 跳过这些接口实现类，会用其他方式来处理这些依赖
        ...
        // 修正依赖，注册一些自动装配的特殊规则，如果是BeanFactory类类型的实现类，在运行时指定为当前BeanFactory
        ...
        // 注册早期后置处理器，用于检测bean是否时ApplicationListener
        beanFactory.addBeanPostProcessor(new ApplicationListenerDetector(this));

        // Detect a LoadTimeWeaver and prepare for weaving, if found.
        // 如果有LOAD_TIME_WEAVER_BEAN_NAME
    ...
    /**
     * 实例化并调用所有已注册的BeanFactoryPostProcessor的Bean，并按已有顺序执行
     * <p>
    ...
    protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
        ...
        // 如果找到LOAD_TIME_WEAVER_BEAN_NAME，就设置后置处理器
    ...
    protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
        ...
        // 初始化容器转换器，通过配置给bean实例成员变量赋值时的类型转换工作
        ...
        // 如果没有注册过bean的后置处理器，则注册默认的解析器，主要用于解析配置文件的PropertyPlaceholderConfigurer
        // @value或xml中使用${}的方式进行环境相关配置
        ...
        // 先初始化 LoadTimeWeaverAware bean，以便尽早注册他们的转换器
